...
const PriceDisplay: React.FC<{
    ...
    OrderData: ProductInterface | null;
}> = ({ PromoCode, OrderData }) => {
    if (PromoCode && OrderData) {
        if (PromoCode.type === "FIXED") {
            ...
            if (!isNaN(cost)) {
                ...
                return Math.round(
                    Number(cost * (Number(PromoCode.value) / 100))
                );
            } else {
...
const OrderComponent: FunctionComponent<OrderComponentProps> = ({
    ...
    const promoPrice = PriceDisplay({
        PromoCode,
        OrderData: infoAboutOrder,
    }) as number;
    ...
    useEffect(() => {
        const config = {
            params: {
                id: id,
                type: currentPath.includes("product")
                    ? "PRODUCT"
    ...
    const handleRedirectOrder = () => {
        var ordersCreateUrl =
            `/order/create?type=${
                infoAboutOrder?.product_type
            }&id=${id}&is_deluxe=${searchParams.get("is_deluxe") ?? false}` +
            "&platform_id=" +
            (searchParams.get("platform_id") ??
                infoAboutOrder?.platforms[0]?.id ??
                "");
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            <div
                ...
                style={{ maxWidth: is_mobile ? "325px" : "-" }}
            >
                <div
                    style={{
                        display: "flex",
                ...
                >
                    ...
                    <span>
                        {Math.max(
                            0,
                            finalPrice -
                                (searchParams.get("is_deluxe") === "true"
                                    ? infoAboutOrder?.deluxe_or_premium_discount
                                          ?.discount_cost ?? 0
                                    : infoAboutOrder?.discount?.discount_cost ??
                                      0)
                        )}{" "}
                        ₽
                    </span>
                ...
                >
                    <span style={{ fontFamily: "Unbounded_Light_Base" }}>
                        Товаров на сумму:
                    </span>
                    <span>{cost} ₽</span>
                </div>
                ...
                >
                    ...
                    <span style={PromoCode ? { color: "#04E061" } : {}}>
                        {promoPrice +
                            (searchParams.get("is_deluxe") === "true"
                                ? infoAboutOrder?.deluxe_or_premium_discount
                                      ?.discount_cost ?? 0
                ...
                {PromoCode ? (
                    ...
                    >
                        <Input
                            placeholder="Есть промокод?"
                            style={{ width: "337px" }}
                            onChange={(event) => setInputPromoCode(event)}
                            maxlenght={50}
                            onEventEnterPressed={handleEnterPromoCode}
                        />
                        <div
                            style={{
                                width: "32px",
                        ...
                        >
                            <img
                                src={"/icons/bases/arrows/arrow_left.svg"}
                                style={{
                                    cursor: "pointer",
                                    scale: "1.5",
                                    margin: "auto",
                ...
                <button
                    style={{ fontFamily: "Unbounded_Medium", fontSize: "12px" }}
                    className={
                        redirect
                            ? styles.button
                            : isAcceptRule
                    ...
                    onClick={redirect ? handleRedirectOrder : handleCreateOrder}
                >
