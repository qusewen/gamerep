...
const Register: FC<ChildProps> = ({
    ...
    const Register = async () => {
        ...
        if (Password.length < 8 || !isNaN(Number(Password))) {
            SetError(
                "Пароль должен быть не менее 8 символов и не должен состоять только из чисел"
            );
            return;
        }
        ...
        try {
            ...
            if (typeof err.response.data === "object") {
                ...
                SetError(
                    errData.map(([key, value]) => `${key}: ${value}`).join(", ")
                );
            } else {
