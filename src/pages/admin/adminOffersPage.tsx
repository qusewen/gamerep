...
const AdminOffersPage: FunctionComponent = () => {
    ...
    >([]);
    const [listCountryWallet, setListCountryWallet] = useState<
    ...
    useEffect(() => {
        ...
        const getListProduct = async () => {
            try {
                const response = await axios.get(
                    "/api/v1/products?page_size=10000&ordering=-pk",
                    {
                        headers: {
                ...
                setListProduct([
                    { value: null, label: "ВСЕ", type: "PRODUCT" },
                    { value: null, label: "ВСЕ", type: "SUBSCRIPTION" },
                    ...response.data.results.map((item: ProductInterface) => ({
                        value: item.id,
                        label: item.title,
        ...
        const getListCountryWallet = async () => {
            try {
                const response = await axios.get(
                    "/api/v1/wallet/country_wallet/"
                );
                setListCountryWallet([
                    { value: null, label: "ВСЕ" },
                    ...response.data.map((item: ProductInterface) => ({
                        value: item.id,
                        label: item.title,
                    })),
                ]);
    ...
    const [reactSelectEmail, setreactSelectEmail] = useState<
        ReactSelectEmail[]
    >([]);
    ...
    useEffect(() => {
        const fetchStaffs = async () => {
            ...
            try {
                ...
                const emailOptions: ReactSelectEmail[] = response.data.map(
                    (user: UserInterface) => ({
                        value: user.pk,
                        label: user.email,
                    })
                            ...
                            <div className={styles.general}>
                                ...
                                >
                                    ...
                                    <FlagButton
                                        ...
                                        text="Все промокоды"
                                        initialState={filterAllFeedback}
                                        onChangeActive={HandleSelectAllFeedback}
                                        onChangeDirectionSorting={
                                            setOrderingAllFeedback
                                        }
                                    />
                                    <FlagButton
                                        className={styles.flag_button}
                                        text="Активные"
                                        initialState={filterPublicFeedback}
                                        onChangeActive={
                                            HandleSelectPublicFeedback
                                        }
                                        onChangeDirectionSorting={
                                            setOrderingPublicFeedback
                                        }
                                    />
                                    <FlagButton
                                        className={styles.flag_button}
                                        text="Скрытые"
                                        initialState={filterHideFeedback}
                                        onChangeActive={
                                            HandleSelectHideFeedback
                                        }
                                        onChangeDirectionSorting={
                                            setOrderingHideFeedback
                                        }
                                    />
                                </div>
                    ...
                    >
                        ...
                        <div className={styles.general}>
                            ...
                            >
                                {newProduct !== null && (
                                    <AdminOffersPanel
                                        listCountryWallet={listCountryWallet}
                                        listProduct={listProduct}
                                        usersOptions={reactSelectEmail}
                                        promocode={newProduct}
                                        funcDelete={() => {
